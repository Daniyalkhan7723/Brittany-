...
class PagingSource(
...
) : PagingSource<Int, WorkoutData>() {
    @OptIn(ExperimentalCoilApi::class)
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, WorkoutData> {
        return try {
            val requestPage = params.key ?: 1 // default page to 1
            val jsonObject = JsonObject().apply {
                if (screenType == Constants.SCREEN_ALL_WORKOUTS_BY_CATEGORY) {
                    addProperty("category_id", categoryId.toInt())
                }
                addProperty("page", requestPage)
                addProperty("search", query)
            }
            val response =
                when (screenType) {
                    Constants.SCREEN_ALL_WORKOUTS_BY_CATEGORY -> apiInterface.workoutByCategory(
                        token = "Bearer $bearerToken",
                        body = jsonObject
                    )

                    Constants.SEARCH_ALL_WORKOUT_FAVOURITES -> apiInterface.allFavouriteWorkouts(
                        token = "Bearer $bearerToken",
                        search = query,
                        page = requestPage
                    )

                    Constants.VIEW_ALL_WORKOUT_FAVOURITES -> apiInterface.allFavouriteWorkouts(
                        token = "Bearer $bearerToken",
                        search = query,
                        page = requestPage
                    )

                    else -> apiInterface.workoutAll(
                        token = "Bearer $bearerToken",
                        body = jsonObject
                    )
                }
    ...
    override fun getRefreshKey(state: PagingState<Int, WorkoutData>): Int? {
